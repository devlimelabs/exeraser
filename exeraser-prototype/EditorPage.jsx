...
const EditorPage = () => {
    ...
    if (file && file.type.startsWith('image/')) {
      try {
        ...
        if (uploadResult.success) {
          setUploadedImage(uploadResult.image_data)
          setFileId(uploadResult.file_id)
          
          // Automatically start person detection
          setTimeout(() => {
            detectPeople(uploadResult.file_id)
          }, 1000)
        } else {
  ...
  const detectPeople = async (fileId) => {
    try {
      setError(null)
      const detectionResult = await imageAPI.detectPeople(fileId)
      
      if (detectionResult.success) {
        setDetectedPeople(detectionResult.people)
  ...
  const handleDownload = () => {
    if (resultId) {
      const downloadUrl = imageAPI.getDownloadUrl(resultId)
